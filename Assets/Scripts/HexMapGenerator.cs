...
public class HexMapGenerator : MonoBehaviour {
    ...
    void EvolveClimate(int cellIndex)
    {
        ...
        float cloudDispersal = cellClimate.clouds / (5f + windStrength);
        float runoff = cellClimate.moisture * runoffFactor / 6f;
        float seepage = cellClimate.moisture * seepageFactor / 6f;
        for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++)
        {
            HexCell neighbor = cell[d];
            if (!neighbor)
    ...
    void CreateRivers()
    {
        List<HexCell> riversOrigins = ListPool<HexCell>.Get();
        for (int i = 0; i < cellCount; i++)
        {
            HexCell cell = grid.GetCell(i);
            if (cell.IsUnderwater)
        ...
        {
            int index = Random.Range(0, riversOrigins.Count);
            int lastIndex = riversOrigins.Count - 1;
            HexCell origin = riversOrigins[index];
